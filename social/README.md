...
[`components/HomeFeed.jac`](components/HomeFeed.jac):
...
[`components/AuthDialog.jac`](components/AuthDialog.jac) does three awaited
...
a window event, and `<SignInDialog />` (mounted once in `components/SocialLayout.jac`) opens
...
```
main.jac                        entry: server import + the routed app shell
jac.toml                        project, theme, JWT secret, microservices off
services/social.sv.jac          THE FILE: nodes, edges, endpoints, load_feed, seed

components/SocialLayout.jac     three-column shell (rail / feed / sidebar) + <Outlet />
components/HomeFeed.jac         /             public_timeline (a scan) OR load_feed (a traversal)
components/ExplorePeople.jac    /explore      discover_people
components/UserProfile.jac      /u/:username  profile page; every stat is an edge count
components/AuthDialog.jac       requireSignIn() + the sign-in/sign-up dialog
components/AuthGate.jac         session state shared through context
components/AccountMenu.jac      who you are, or a Sign in button
components/PostCard.jac         the like button -> toggles a Likes-edge
components/FollowButton.jac     follow/unfollow -> one Follows-edge
components/PostComposer.jac     create_post -> a Posted-edge
components/LeftNav.jac          the rail: 72px icons, 275px with labels
components/RightSidebar.jac     who-to-follow + trending, both read off the graph
components/MobileBar.jac        the < 768px top bar
components/ui/                  jac-shadcn primitives (yours to edit)

lib/utils.jac                   cn()
lib/format.cl.jac               timeAgo(), initials() — pinned client: browser Date, no JSX/npm seed
